...
- Python `>=3.12,<3.13`. Install with dev deps: `uv sync --dev`.
...
Focused test: `uv run pytest tests/test_smoke.py::test_create_app`.
...
- Imports use top-level packages such as `from config.loader import ...`, not `from src...`; pytest adds `src` to `pythonpath`.

## Config

- Centralized in `src/config/loader.py`; path precedence is explicit loader argument → `APP_CONFIG_PATH` → `config/app.yaml` (gitignored — start from `config/app.yaml.template`).
...
- Unit tests use no real DB or network. Tests requesting the `postgres_engine` fixture start testcontainers Postgres and auto-skip when Docker is unavailable; this is fixture-driven, not marker-driven.
