...
class IndexHistGlobalPipeline(IngestionPipeline):
    """Pipeline for fetching global index history from Akshare."""

    retry_policy: RetryPolicy
    engine: Engine
    index_info_table: Table
    codes_raw: str
    _fetcher: AkshareHkIndexDailyFetcher = field(init=False, repr=False)
    _cleaner: IndexHistGlobalCleaner = field(init=False, repr=False)
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "_fetcher", AkshareHkIndexDailyFetcher(self.retry_policy))
        object.__setattr__(self, "_cleaner", IndexHistGlobalCleaner())
    ...
    def fetch(self, chunk_args: ChunkArgs) -> RawBatch:
        """Fetch raw global index history for the date range."""
        return self._fetcher.fetch(chunk_args)

    def clean(self, raw_batch: RawBatch) -> NormalizedBatch:
        """Clean raw global index history into index_hist records."""
    ...
    def _ensure_initialized(self) -> None:
        ...
        object.__setattr__(self, "_codes", build_code_mappings(codes, _convert_global_code))
...
def _convert_global_code(code: str) -> str:
    mapping = {
        "HKTECH.GLB": "HSTECH",
    }
    return mapping.get(code, strip_suffix(code))
