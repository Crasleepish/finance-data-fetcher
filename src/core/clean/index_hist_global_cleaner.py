...
class IndexHistGlobalCleaner:
    """Normalize Tushare index_global rows into index_hist records."""

    def clean(self, raw_batch: RawBatch) -> NormalizedBatch:
        """Normalize raw index_global rows into DB-ready records."""
        ...
        for row in _list_of_dicts(raw_batch):
            ...
            record = {
                ...
                "date": _parse_date(row.get("date") or row.get("trade_date")),
                ...
                "volume": _as_int(row.get("volume") or row.get("vol")),
                "amount": _as_float(row.get("amount")),
                "change_percent": _as_float(row.get("pct_chg")),
